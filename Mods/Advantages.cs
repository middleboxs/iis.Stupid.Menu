...
public static void UltraInstinct() /* my first time making a mod, really crap code, also doing it on a school chromebook so can't even run it to test if it works*/
        {
            foreach (VRRig vrrig in GorillaParent.instance.vrrigs)
            {
                Vector3 they = vrrig.headMesh.transform.position;
